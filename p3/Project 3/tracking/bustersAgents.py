...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Get the position of each ghost with max probabilities
        for ghost in livingGhostPositionDistributions:
            for pos, prob in ghost.items():
                if prob > max_prob:
                    max_prob = prob
                    max_pos = pos
            list_of_ghosts_pos.append(max_pos)
            max_prob = -1

        successorPositions = []
        for legal_move in legal:
            successorPositions.append((legal_move, Actions.getSuccessor(pacmanPosition, legal_move)))

        list_of_dist = [self.distancer.getDistance(pacmanPosition, pos_ghost) for pos_ghost in list_of_ghosts_pos]
        index_of_min_dist = list_of_dist.index(min(list_of_dist))
        closest_ghost = list_of_ghosts_pos[index_of_min_dist]

        min_dist = 9999
        move_choice = ''
        # Now choose which legal action to take
        for legal_move, pos in successorPositions:
            dist_now = self.distancer.getDistance(pos, closest_ghost)
            if dist_now < min_dist:
                min_dist = dist_now
                move_choice = legal_move
