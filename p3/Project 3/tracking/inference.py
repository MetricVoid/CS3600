...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        THE FORMULAS:
        P(TrueDistance) is the old self.beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        pacmanPosition = gameState.getPacmanPosition()
        """
        Want to get: Pr(Ghost is at position p at time t + 1) =
        Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t ) * P(ghost is at position oldPos at time t)
        """
        newPosDist = {}
        ...
        for oldPos, prob in newPosDist.items():
            for newPos in prob:
                allPossible[newPos] += prob[newPos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particle_positions = [(0, 0)] * self.numParticles
        index = 0
        for i in range(self.numParticles):
            self.particle_positions[i] = self.legalPositions[index]
            index += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for p in self.particle_positions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                updated_beliefs[p] += emissionModel[trueDistance]
        else:
            self.particle_positions = [self.getJailPosition()] * self.numParticles
            return
        updated_beliefs.normalize()

        # If we get a zero weight distribution...
        if updated_beliefs.totalCount() == float(0):
            self.initializeUniformly(gameState)
        else:
            self.particle_positions = [util.sample(updated_beliefs) for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.legalPositions:
            ...
            for newPos in newPosDist:
                updated_beliefs[newPos] += newPosDist[newPos] * beliefs[p]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.product_legal_positions = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(self.product_legal_positions)
        self.particles = [None] * self.numParticles
        index = 0
        for i in range(self.numParticles):
            self.particles[i] = self.product_legal_positions[index]
            index += 1
            if index >= len(self.product_legal_positions):
                index = 0
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        "*** YOUR CODE HERE ***"
        updated_beliefs = util.Counter()
        for p_index, p in enumerate(self.particles):
            temp = 1
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] is not None:
                    true_distance = util.manhattanDistance(pacmanPosition, p[ghost])
                    temp *= emissionModels[ghost][true_distance]
                else:
                    p = self.getParticleWithGhostInJail(p, ghost)

            updated_beliefs[p] += temp

        updated_beliefs.normalize()

        if updated_beliefs.totalCount() == 0.0:
            self.initializeParticles()
        else:
            self.particles = [util.sample(updated_beliefs) for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            updated_beliefs = util.Counter()
            for p_index, p in enumerate(newParticle):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), p_index, self.ghostAgents[p_index])
                newParticle[p_index] = util.sample(newPosDist)
            "*** END YOUR CODE HERE ***"
