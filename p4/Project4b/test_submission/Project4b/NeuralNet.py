...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """
        """YOUR CODE"""
        inActsDummy = list(inActs)
        inActsDummy.insert(0, 1)
        result_list = []
        for i in range(len(inActsDummy)):
            result_list.append(inActsDummy[i] * self.weights[i])
        return self.sigmoid(sum(result_list))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        activation_value = self.sigmoidActivation(inActs)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(self.weights)):
            error_shit = inActs_copy[i] * delta
            self.weights[i] += (alpha * error_shit)
            totalModification += abs(alpha * error_shit)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        """
        """YOUR CODE"""
        result = [[] for _ in range(len(self.layerSize))]

        # insert the input layer
        result[0] += inActs

        # forward propagate for hidden layers
        for i in range(self.numHiddenLayers):
            for j in range(len(self.hiddenLayers[i])):
                result[i + 1].append(self.hiddenLayers[i][j].sigmoidActivation(inActs))
            inActs = result[i + 1]

        # forward propagate the output layer
        for i in range(len(self.outputLayer)):
            result[-1].append(self.outputLayer[i].sigmoidActivation(inActs))
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            allLayerOutput = self.feedForward(example[0]) # fill in
            ...
            for outputNum in xrange(len(example[1])):
                ...
                error = example[1][outputNum] - lastLayerOutput[outputNum] # fill in
                delta = error * gPrime # fill in
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    temp = 0
                    # based on the formula, have to do this
                    for i in xrange(len(nextLayer)):
                        temp += nextLayer[i].weights[neuronNum + 1] * deltas[0][i]
                    delta = gPrime * temp
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                for numNeuron in xrange(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(inActs=allLayerOutput[numLayer], alpha=alpha, delta=deltas[numLayer][numNeuron])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    """
    Stop condition: averageWeightMod < weightChangeThreshold = 0.00008 or iteration > maxItr
    """
    ...
    for index_test, test in enumerate(examplesTest):
